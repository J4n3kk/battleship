...
const GameboardFactory = function () {
  ...
  function isAdjecentFree (coord) {
    ...
    const isLocationAvailable = locations.map(x => (x === undefined || x.length === 0) ? true : false )
    
    
    

    return isLocationAvailable.filter(x => x === false).length > 0 ? false : true
  }
  const gameboardController = {
    placeShip: function(...coords){
      // input validation
        // check if input is numeric
        let atomicInput = coords.flat(Infinity)
      ...
      // board validation
        ...
        const isAvailable = shipLocation.filter(location => location.length > 0).length === 0 ? true : false
        if (isAvailable === false) return false
        // check if other ships does not already exist in adjecent spots to coords


        const isLocationFree = coords.map(coord => isAdjecentFree(coord)).includes(false) ? false : true
